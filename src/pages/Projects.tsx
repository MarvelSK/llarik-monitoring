...
const Projects = () => {
  ...
  const filteredProjects = projects.filter(project => project.name.toLowerCase().includes(searchTerm.toLowerCase()) || project.description && project.description.toLowerCase().includes(searchTerm.toLowerCase()));
  ...
  return <Layout>
      <div className="space-y-6">
        ...
        {!loading && projects.length > 0 && <div className="flex flex-col md:flex-row md:items-center gap-4">
            <div className="relative w-full md:w-96">
              <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 w-4 h-4" />
              <Input placeholder="Vyhľadať projekt..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="pl-9" />
            </div>
            <div className="flex items-center ml-auto gap-2">
              <span className="text-sm text-muted-foreground">Zobraziť ako:</span>
              <Button variant={viewMode === "table" ? "default" : "outline"} size="sm" onClick={() => setViewMode("table")} className="h-8">
                Tabuľka
              </Button>
              <Button variant={viewMode === "grid" ? "default" : "outline"} size="sm" onClick={() => setViewMode("grid")} className="h-8">
                Karty
              </Button>
            </div>
          </div>}
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
            <ScrollArea className="h-[calc(100vh-280px)]">
              <Table>
                ...
                <TableBody>
                  {filteredProjects.map(project => <TableRow key={project.id} className={`${selectedProject === project.id ? 'bg-primary/5' : ''}`}>
                      <TableCell className="font-medium cursor-pointer" onClick={() => handleSelectProject(project)}>
                        {project.name}
                      </TableCell>
                      <TableCell className="hidden md:table-cell max-w-xs truncate cursor-pointer" onClick={() => handleSelectProject(project)}>
                        {project.description || "—"}
                      </TableCell>
                      <TableCell className="hidden sm:table-cell cursor-pointer" onClick={() => handleSelectProject(project)}>
                        {new Date(project.createdAt).toLocaleDateString()}
                      </TableCell>
                      <TableCell>
                        <div className="flex flex-col">
                          {project.ownerId === currentUserId ? <Badge variant="outline" className="bg-primary/10 text-primary border-primary/30 text-xs">
                              Vlastník
                            </Badge> : isAdmin ? <Badge variant="outline" className="bg-yellow-50 text-yellow-700 border-yellow-200 text-xs">
                              Admin prístup
                      ...
                      <TableCell className="text-right space-x-1">
                        ...
                        {(project.ownerId === currentUserId || isAdmin) && <>
                            <Button variant="ghost" size="sm" onClick={() => openAddMemberDialogFor(project)} className="h-8">
                      ...
                      setOpenEditDialog(true);
                            ...
                            <Button variant="ghost" size="sm" className="h-8 text-red-500 hover:text-red-600" onClick={() => {
                      setEditProject(project);
      ...
      <div className="grid gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {filteredProjects.map(project => <Card key={project.id} className={`overflow-hidden cursor-pointer hover:shadow-md transition-shadow ${selectedProject === project.id ? 'ring-2 ring-primary' : ''}`} onClick={() => handleSelectProject(project)}>
                <CardHeader className="pb-2 space-y-0">
                  <div className="flex justify-between items-start">
                    <CardTitle className="text-base">
                      {project.name}
                    </CardTitle>
                    {(project.ownerId === currentUserId || isAdmin) && <Button variant="ghost" size="icon" className="h-7 w-7" onClick={e => {
                e.stopPropagation();
                setEditProject(project);
                setOpenEditDialog(true);
              }}>
                ...
                <CardContent className="pb-2">
                  <div className="mb-1 flex gap-1">
                    ...
                    {isAdmin && project.ownerId !== currentUserId && <Badge variant="outline" className="bg-yellow-50 text-yellow-700 border-yellow-200 text-xs">
                        Admin prístup
                      </Badge>}
                  </div>
        ...
        {selectedProject && <Card className="sticky bottom-6 md:hidden border-primary/40 shadow-md">
            ...
            <CardContent className="pb-3 pt-0">
              <div className="flex flex-wrap gap-2">
                <Button size="sm" onClick={() => navigateToProjectChecks(projects.find(p => p.id === selectedProject)!)} className="h-8">
                  Zobraziť kontroly
                </Button>
                {(isProjectOwner(selectedProject) || isAdmin) && <Button variant="outline" size="sm" onClick={() => openAddMemberDialogFor(projects.find(p => p.id === selectedProject)!)} className="h-8">
